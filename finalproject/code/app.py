...
def Register():
 ...
 if request.method == 'POST' and 'email' in request.form and 'username' in request.form and 'password' in request.form:
    ...
    cursor.execute("Select * from userdata where emailid LIKE %s", [email])
    user = cursor.fetchone()

    if user:
       flash("user already exist", "Danger")
    else:
       query = "insert into userdata(name,emailid,password) values(%s,%s,%s)"
...
def Login():
 ...
 if request.method == 'POST' and 'email' in request.form and 'password' in request.form:
    ...
    cursor = mySqlConnection.connection.cursor(MySQLdb.cursors.DictCursor)
    user = cursor.execute(
        "Select * from userdata where userdata.emailid =  %s", [email])
    error = None
...
def FileUploader():
   ...
   if request.method == 'POST':
      ...
      with open("./static/emails.json","w") as jsonfile:
      ...
      f.save(filepath)
      s3_client.meta.client.upload_file(filepath,configData['BUCKET_NAME'],"1.png")				  
         
      s3_client.meta.client.upload_file(emailfilepath,configData['BUCKET_NAME'],"emails/emails.json")
...
def GetFiles():
    ...
    if userid is not None:
        cursor = mySqlConnection.connection.cursor(MySQLdb.cursors.DictCursor)
        files=cursor.execute("Select * from filedata where id = %s",[userid]).fetchall()
        jlist=None
